...
servicetimes::Array{Int64,2})
    ...
    for saving in savings
        ...
        elseif feasibility == "feasible"
            ...
            # Calculate sum ( length(truckRoutes)^3 ) before
            routes = Int64[]
            for route in currentRoutes
                val = length(route)^3
                push!(routes, val)
            end
            truckOb_before = sum(routes)
            ...
            # Calculate sum ( length(truckRoutes)^3 ) after
            routes = Int64[]
            for route in currentRoutes
                val = length(route)^3
                push!(routes, val)
            end
            truckOb_after = sum(routes)

            # Calculate savings
            truckSave = truckOb_after - truckOb_before
