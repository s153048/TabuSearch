...
servicetimes::Array{Int64,2})
    ...
    for saving in savings
        ...
        elseif feasibility == "feasible"
            ...
            # Update routes
            returnRoute[routeNum] = tempRoute
            deleteat!(returnRoute[currentTruck], u)

            # Format return for TABU
            customers = [customer]

            # Disband route if it has only 2 sites left (2x depot)
            if length(returnRoute[currentTruck]) == 2
                truckSave = currentObjVal/length(returnRoute)
                deleteat!(returnRoute, currentTruck)
            else
                truckSave = 0::Int64
            end

            returnObjVal = currentObjVal - saving[1] - truckSave
