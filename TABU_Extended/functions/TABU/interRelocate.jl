...
#   tabuQueue::Queue{Array{Array{Int64,1},1}}(TabuQueue)
# OUTPUT:
...
servicetimes::Array{Int64,2}, tabuQueue::Queue{Array{Array{Int64,1},1}})

    # Initialize
    curRoute = currentRoutes[currentTruck]
    u = findall(x->x==customer,curRoute)[1]
    savings =  Tuple{Float64,Int64,Int64,Float64}[]
    ...
    for newRoute in currentRoutes
        ...
        if newRoute == curRoute
            ...
            for i in 1:length(newRoute)-1
                ...
                # Adding more weight to savings moving vehicles from a shorter to a longer route
                moveWeight = 0.0
                if length(newRoute) > length(curRoute)
                    moveWeight = deepcopy(saving)
                    saving = saving^2
                end

                # Push saving, insertion spot and routenumber
                push!(savings, (saving, j, routeNum, moveWeight))
            end
        end
    end
    ...
    for saving in savings
        ...
        insert!(tempRoute, insSpot, customer)
        ...
        elseif feasibility == "feasible"
            ...
            deleteat!(checkRoutes[currentTruck], u)

            if in(checkRoutes, tabuQueue)
                # Aspiration check
                # Disband route if it has only 2 sites left (2x depot)
                if length(checkRoutes[currentTruck]) == 2
                    truckSave = currentObjVal/length(currentRoutes)
                    deleteat!(checkRoutes, currentTruck)
                else
                    truckSave = 0::Int64
                end

                # Check if weight was used
                if saving[4] != 0
                    checkObj = currentObjVal - saving[4] - truckSave
                elseif saving[4] == 0
                    checkObj = currentObjVal - saving[1] - truckSave
                end

                if checkObj < bestObjVal
                    # Better than all-time best. Choose
                    currentRoutes = checkRoutes
                    currentObjVal = checkObj
                    return feasibility, currentRoutes, currentObjVal
                else
                    continue
                end
            else
                ...
                # Check if weight was used
                if saving[4] != 0
                    currentObjVal = currentObjVal - saving[4] - truckSave
                elseif saving[4] == 0
