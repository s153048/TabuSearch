...
function runTABU_iterationAnalysis(truckRoutes::Array{Array{Int64,1},1}, objValue::Float64,
k::Any, runtime::Int64, distanceMatrix::Array{Float64,2},
timewindows::Array{Float64,2}, capacity::Int64, demands::Array{Int64,2},
vehicles::Int64, servicetimes::Array{Int64,2}, T::Int64)
    ...
    let
        ...
        while round( (time_ns()-startTime)/1e9,digits=3) < runtime
            ...
            if operator == "intraRelocate"
                ...
                # Run operator
                feasibility, currentRoutes, currentObjVal =
                intraRelocate(currentRoutes, currentObjVal, truck, customer,
                distanceMatrix, timewindows, servicetimes, tabuQueue, bestObjVal)
            ...
            # Intensification
            if t >= T
                # Go back to previous best
                currentRoutes = deepcopy(bestRoutes)
                currentObjVal = deepcopy(bestObjVal)
                # Reset Tabu list
                tabuQueue = Queue{Array{Array{Int64,1},1}}()
            end
