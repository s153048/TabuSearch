...
function runTABU_operatorAnalysis(truckRoutes::Array{Array{Int64,1},1}, objValue::Float64,
...
vehicles::Int64, servicetimes::Array{Int64,2})
    ...
    let
        ...
        while round( (time_ns()-startTime)/1e9,digits=3) < runtime
            ...
            if operator == "intraRelocate"
                operatorCount[1] += 1
                push!(intraRval, beforeOpObjVal-currentObjVal)
            elseif operator == "intraTwoOpt"
                operatorCount[2] += 1
                push!(intraTval, beforeOpObjVal-currentObjVal)
            elseif operator == "interRelocate"
                operatorCount[3] += 1
                push!(interRval, beforeOpObjVal-currentObjVal)
            elseif operator == "interExchange"
                operatorCount[4] += 1
                push!(interEval, beforeOpObjVal-currentObjVal)
            end
    ...
    return bestRoutes, bestObjVal, iterations, operatorCount, intraRval, intraTval, interRval, interEval
end
