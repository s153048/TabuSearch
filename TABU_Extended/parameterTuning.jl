...
function parameterTuning()
    ...
    function tuneTABU(seed, instance, runtime, k, T)
        ...
        truckRoutes = insertionHeuristic(parameters[6], name, vehicles, capacity, demands, timewindows, servicetimes, distanceMatrix)
        objVal = objectiveValue(truckRoutes, distanceMatrix)

        # Run TABU
        bestRoutes, bestObjVal = runTABU(truckRoutes, objVal, k, runtime, distanceMatrix, timewindows, capacity, demands, vehicles, servicetimes, T)
    ...
    runtime = 10
    timelimit = 7200
    # 1 iteration = Instances * runtime * samplesize = 4 x 3 x 20 = 360 seconds (6 min) x 40 iterations = 7200 seconds (2 hours)
    ...
    TABU(seed, instance, k, T) = (tuneTABU(seed, instance, runtime, k, T) - bestKnown[instance])/bestKnown[instance]


    # Conduct tuning
    cmd = AlgoTuner.createRuntimeCommand(TABU)
    AlgoTuner.addIntParam(cmd, "k", 5, 150)
    AlgoTuner.addIntParam(cmd, "T", 50, 5000)
