...
function deviation_analysis()



    # Construct dataframe to hold results
    df = DataFrame(Filename = String[], Seed = Int64[], bnDist = Float64[], bnVehi = Int64[],
    ...
    files = ["C1_2_2.TXT", "C2_2_2.TXT", "R1_2_2.TXT", "R2_2_2.TXT"]
    seeds = [1, 2, 3]
    for file in files
        ...
        for seed in seeds
            ...
            push!(df, (file, seed, bnDist, bnVehi, tabuDist, tabuVehi, dDist, dVehi,
            constructDist, constructVehi, dcDist, dcVehi, iterations, successCheck))
            println("Run: ", count, "/12")
            count += 1
        end
    ...
    CSV.write("output/deviation_analysis.csv", df)
    #writedlm("output/iterationAnalysis.csv", iA_array, ',')
end
