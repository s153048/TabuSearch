...
tabuQueue::Queue{Tuple}, bestObjVal::Float64, demands::Array{Int64,2},
    ...
    for saving in savings
        ...
        elseif feasibility == "feasible"
            ...
            elseif feasibility == "feasible"
                ...
                move1 = (customer, routeNum, swapIDX)
                move2 = (swapCus, currentTruck, u)
                if in(move1, tabuQueue) || in(move2, tabuQueue)
                    continue
                else
                    # Store new tabuMove
                    tabuMoves = [(customer, currentTruck, u) (swapCus, routeNum, swapIDX)]

                    # TABU check succeeded, so routes are updated
                    currentRoutes[currentTruck] = tempRoute1
                    currentRoutes[routeNum] = tempRoute2

                    currentObjVal = currentObjVal - saving[1]

                    return feasibility, currentRoutes, currentObjVal, tabuMoves
                end
            end
        end
    end
